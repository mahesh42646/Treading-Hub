'use client';

import React, { useState, useEffect } from 'react';
import Link from 'next/link';
import { FaCheck, FaTimes } from 'react-icons/fa';

const ChooseChallengeSection = () => {
  const [challenges, setChallenges] = useState([]);
  const [loading, setLoading] = useState(true);
  const [error, setError] = useState('');
  const [selectedType, setSelectedType] = useState('');
  const [selectedModel, setSelectedModel] = useState('');

  useEffect(() => {
    fetchChallenges();
  }, []);

  const fetchChallenges = async () => {
    try {
      const response = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/api/challenges/configs`);
      
      if (response.ok) {
        const data = await response.json();
        setChallenges(data.challenges || []);
      } else {
        setError('Failed to load challenges');
      }
    } catch (error) {
      setError('Network error. Please try again.');
    } finally {
      setLoading(false);
    }
  };

  if (loading) {
    return (
      <section className="py-5" style={{
        background: 'linear-gradient(135deg, #110A28 0%, #110A28 100%)',
        color: 'white',
        minHeight: '50vh'
      }}>
        <div className="container">
          <div className="d-flex justify-content-center align-items-center" style={{ minHeight: '50vh' }}>
            <div className="spinner-border text-white" role="status">
              <span className="visually-hidden">Loading...</span>
            </div>
          </div>
        </div>
      </section>
    );
  }

  return (
    <section className="py-5" style={{
      background: 'linear-gradient(135deg, #110A28 0%, #110A28 100%)',
      color: 'white'
    }}>
      <div className="container">
        <div className="text-center mb-4 mb-md-5 pt-lg-5">
          <h2 className="display-5 display-md-3 display-sm-5 fw-bold mb-3 text-white" style={{ fontSize: 'clamp(1.5rem, 4vw, 3rem)' }}>Choose the Best Challenge</h2>
        </div>

      

     

        {/* Challenges Grid - Dark Glossy Theme */}
        <div className="row g-3 g-md-4 justify-content-center">
          {challenges.slice(0, 3).map((challenge, index) => {
            const gradients = [
              'linear-gradient(135deg, #ff6b6b, #ee5a24)',
              'linear-gradient(135deg, #4834d4, #686de0)',
              'linear-gradient(135deg, #6c5ce7, #a29bfe)'
            ];
            
            return (
              <div key={challenge._id} className="col-12 col-sm-6 col-lg-4">
                <div className="card border-0 rounded-4 h-100 position-relative overflow-hidden" style={{
                  background: 'rgba(60, 58, 58, 0.03)',
                  border: '3px solid rgba(67, 34, 124, 0.74)',
                  backdropFilter: 'blur(20px)',
                  boxShadow: '5px 4px 20px 1px rgba(70, 74, 103, 0.44)',
                  transition: 'transform 0.3s ease, box-shadow 0.3s ease'
                }}
                onMouseEnter={(e) => {
                  e.currentTarget.style.transform = 'translateY(-10px)';
                  e.currentTarget.style.boxShadow = '0 20px 40px rgba(0, 0, 0, 0.4)';
                }}
                onMouseLeave={(e) => {
                  e.currentTarget.style.transform = 'translateY(0)';
                  e.currentTarget.style.boxShadow = '5px 4px 20px 1px rgba(70, 74, 103, 0.44)';
                }}>
                  
                  {/* Gradient Header */}
                  <div className="position-relative" style={{
                    background: gradients[index % gradients.length],
                    minHeight: 'clamp(100px, 15vw, 120px)',
                    display: 'flex',
                    alignItems: 'center',
                    justifyContent: 'center',
                    flexDirection: 'column',
                    padding: 'clamp(1rem, 3vw, 2rem) clamp(0.5rem, 2vw, 1rem) clamp(0.5rem, 2vw, 1rem)'
                  }}>
                    {/* Decorative dots */}
                    <div className="position-absolute" style={{
                      top: '10px',
                      right: '10px',
                      display: 'flex',
                      gap: '4px'
                    }}>
                      {[...Array(3)].map((_, i) => (
                        <div key={i} style={{
                          width: '4px',
                          height: '4px',
                          borderRadius: '50%',
                          background: 'rgba(255, 255, 255, 0.6)'
                        }}></div>
                      ))}
                    </div>
                    
                    <h3 className="text-white fw-bold mb-2" style={{ fontSize: 'clamp(1.1rem, 3vw, 1.5rem)' }}>
                      {challenge.name}
                    </h3>
                    <div className="text-center">
                      <span className="fw-bold text-white" style={{ fontSize: 'clamp(1.5rem, 4vw, 2.5rem)' }}>{challenge.durationDays} Days</span>
                      <span className="text-white ms-1 ms-md-2" style={{ fontSize: 'clamp(0.7rem, 2vw, 0.9rem)' }}>DURATION</span>
                    </div>
                  </div>

                  {/* Card Body */}
                  <div className="card-body p-3 p-md-4" style={{ 
                    background: 'rgba(60, 58, 58, 0.03)',
                    border: '3px solid rgba(67, 34, 124, 0.74)',
                    backdropFilter: 'blur(20px)',
                    boxShadow: '5px 4px 20px 1px rgba(70, 74, 103, 0.44)'
                  }}>
                    <div className="mb-3 mb-md-4">
                      <div className="d-flex align-items-center mb-2">
                        <div className="me-2 me-md-3" style={{ flexShrink: 0 }}>
                          <FaCheck className="text-success" size={16} />
                        </div>
                        <span className="text-white-50" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.9rem)', lineHeight: '1.3' }}>
                          {challenge.profitTargets?.[0] || 8}% Profit Target
                        </span>
                      </div>
                      <div className="d-flex align-items-center mb-2">
                        <div className="me-2 me-md-3" style={{ flexShrink: 0 }}>
                          <FaCheck className="text-success" size={16} />
                        </div>
                        <span className="text-white-50" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.9rem)', lineHeight: '1.3' }}>
                          {challenge.type} Challenge
                        </span>
                      </div>
                      <div className="d-flex align-items-center mb-2">
                        <div className="me-2 me-md-3" style={{ flexShrink: 0 }}>
                          <FaCheck className="text-success" size={16} />
                        </div>
                        <span className="text-white-50" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.9rem)', lineHeight: '1.3' }}>
                          {challenge.model} Model
                        </span>
                      </div>
                      {challenge.platforms && challenge.platforms.length > 0 && (
                        <div className="d-flex align-items-center mb-2">
                          <div className="me-2 me-md-3" style={{ flexShrink: 0 }}>
                            <FaCheck className="text-success" size={16} />
                          </div>
                          <span className="text-white-50" style={{ fontSize: 'clamp(0.75rem, 2vw, 0.9rem)', lineHeight: '1.3' }}>
                            {challenge.platforms.join(', ')} Platform
                          </span>
                        </div>
                      )}
                    </div>

                    {/* CTA Button */}
                    <Link 
                      href="/challenges" 
                      className="btn w-100 py-2 py-md-3 fw-bold rounded-4 text-white border-0"
                      style={{
                        background: gradients[index % gradients.length],
                        transition: 'all 0.3s ease',
                        fontSize: 'clamp(0.8rem, 2.5vw, 1rem)'
                      }}
                      onMouseEnter={(e) => {
                        e.currentTarget.style.transform = 'scale(1.05)';
                      }}
                      onMouseLeave={(e) => {
                        e.currentTarget.style.transform = 'scale(1)';
                      }}
                    >
                      START CHALLENGE
                    </Link>
                  </div>
                </div>
              </div>
            );
          })}
        </div>

        {error && (
          <div className="text-center mt-3 mt-md-4">
            <div className="alert alert-warning" role="alert" style={{ fontSize: 'clamp(0.8rem, 2vw, 1rem)' }}>
              {error}
            </div>
          </div>
        )}

        {challenges.length === 0 && (
          <div className="text-center mt-3 mt-md-4">
            <div className="alert alert-info" role="alert" style={{ fontSize: 'clamp(0.8rem, 2vw, 1rem)' }}>
              No challenges available at the moment.
            </div>
          </div>
        )}
      </div>
    </section>
  );
};

export default ChooseChallengeSection;
